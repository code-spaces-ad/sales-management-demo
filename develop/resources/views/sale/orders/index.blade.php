{{-- 売上伝票一覧画面用Blade --}}
{{-- @copyright © 2025 CodeSpaces --}}
@extends('layouts.base')
@extends('layouts.head')
@extends('layouts.sidebar')
@extends('layouts.header')
@extends('layouts.footer')

@php
    $headline = config('consts.title.sale.menu.index');
    /** @see MasterCustomersConst */
    $maxlength_customer_code = MasterCustomersConst::CODE_MAX_LENGTH; // 得意先コード最大桁数
    $min_customer_code = MasterCustomersConst::CODE_MIN_VALUE; // 得意先コード最小値

    // デフォルトMAX日付・月
    $default_max_date = config('consts.default.common.default_max_date');
    $default_max_month = config('consts.default.common.default_max_month');
@endphp

@section('title', $headline . ' | ' . config('app.name'))
@section('headline', $headline)

@section('content')
    <div class="row">

        <form name="searchForm" action="{{ route('sale.orders.index') }}" method="GET"
              class="col-12 px-0 px-md-4 pb-md-2">

            {{-- 検索項目 --}}
            <div class="card">
                <div class="card-header">
                    検索項目
                </div>
                <div class="card-body">
                    <div class="form-group d-md-inline-flex col-12 pl-0 pr-0 m-0">
                        {{-- 伝票番号 --}}
                        @component('components.index.order_number')
                            @slot('title', '伝票番号')
                            @slot('search_condition_input_data', $search_condition_input_data)
                            @slot('order_number', 'order_number')
                        @endcomponent

{{--                        --}}{{-- 受注番号 --}}
{{--                        @component('components.index.order_number')--}}
{{--                            @slot('title', '受注番号')--}}
{{--                            @slot('search_condition_input_data', $search_condition_input_data)--}}
{{--                            @slot('order_number', 'orders_received_number')--}}
{{--                        @endcomponent--}}
                    </div>

                    <div class="form-group d-md-inline-flex col-12 pl-0 pr-0 m-0">
                        {{-- 伝票種別 --}}
                        @include('common.index.transaction_type_checkbox')
                    </div>

                    <div class="form-group d-md-inline-flex col-12 pl-0 pr-0 m-0">
                        {{-- 伝票日付 --}}
                        @include('common.index.order_date')

                        {{-- 請求日付 --}}
                        @include('common.index.billing_date')
                    </div>

                    {{-- 得意先 --}}
                    @include('common.index.customer_select_list')

                    <div class="form-group d-md-inline-flex col-12 px-0 m-0">
                        {{-- 支所 --}}
                        @include('common.index.branch_select_list')

                        {{-- 納品先 --}}
                        @include('common.index.recipient_select_list')
                    </div>

                    <div class="form-group d-md-inline-flex col-12 px-0 m-0">
                        {{-- 部門 --}}
                        @include('common.index.department_select_list')
                        {{-- 事業所 --}}
                        @include('common.index.office_facility_select_list')
                    </div>
                </div>
                <div class="card-footer">
                    {{-- 検索・クリアボタン --}}
                    @include('common.index.search_clear_button')
                </div>
            </div>
        </form>

        <div class="col-md-12 result-table-area table-responsive table-fixed" style="max-height: none !important;">
            <table class="table table-bordered table-responsive-org table-list mb-0">
                <thead class="thead-light">
                <tr class="text-center">
                    <th class="border-0" style="background-color: transparent; width: 2%;"></th>
                    <th class="border-0 col-md-10" style="background-color: transparent;"></th>
                    <th class="col-md-1">売上合計</th>
                    <th class="border-0 col-md-1" style="background-color: transparent;"></th>
                </tr>
                <tr style="background-color: transparent;">
                    <td class="border-0" style="background-color: transparent;"></td>
                    <td class="border-0" style="background-color: transparent;"></td>
                    <td class="text-right align-middle border-width"
                        style="background-color: transparent;" data-title="売上合計">
                        {{ number_format($search_result['sales_orders_total']) }}
                    </td>
                    <td class="border-0" style="background-color: transparent;"></td>
                </tr>
                <tr>
                    <td class="border-0" style="height: 10px"></td>
                </tr>
            </table>
        </div>

        <div class="col-md-12">
            <div class="form-group d-md-inline-flex col-12 px-0 m-0">
                <div class="form-group d-md-inline-flex col-md-6 p-0 m-0">
                    {{ $search_result['sales_orders']->appends($search_condition_input_data)->links() }}
                </div>
                <div class="col-md-6 m-0 p-0 text-right">
                    {{-- 新規登録ボタン --}}
                    @component('components.index.create_button')
                        @slot('route', route('sale.orders.create'))
                    @endcomponent
                </div>
            </div>
            <div class="result-table-area table-responsive table-fixed" style="max-height: none !important;">
                <table class="table table-bordered table-responsive-org table-list">
                    <thead class="thead-light">
                    <tr class="text-center">
                        <th scope="col" style="width: 2%;">
                            <a class="centralOpen">+</a>
                            <a class="centralClose float-none" style="display: none;">-</a>
                        </th>
                        <th scope="col" class="d-none">納品書</th>
                        <th scope="col" style="width: 8%;">伝票番号</th>
                        <th scope="col" style="width: 3%;">種別</th>
                        {{--                        <th scope="col" class="col-md-1">受注番号</th>--}}
                        <th scope="col" style="width: 8%;">伝票日付</th>
                        <th scope="col" style="width: 8%;">請求日付</th>
                        <th scope="col" style="width: 16%;">得意先名</th>
                        <th scope="col" style="width: 8%;">支所名</th>
                        <th scope="col" style="width: 16%;">納品先名</th>
                        <th scope="col" style="width: 6%;">部門名</th>
                        <th scope="col" style="width: 8%;">事業所名</th>
                        <th scope="col" style="width: 8%;">金額計(小計)</th>
                        <th scope="col" style="width: 8%;">消費税</th>
                    </tr>
                    </thead>
                    <tbody>
                    @foreach ($search_result['sales_orders'] as $order)
                        <tr>
                            <td class="text-center align-middle" data-title="詳細">
                                <a class="open">+</a>
                                <a class="close float-none" style="display: none;">-</a>
                            </td>
                            {{-- 納品書出力 --}}
                            <td class="text-center align-middle d-none" data-title="納品書">
                                @if ($order->getPrintingDate())
                                    <div class="btn btn-primary mx-1">完</div>
                                @else
                                    <div class="btn btn-secondary mx-1">未</div>
                                @endif
                            </td>
                            <td class="text-center align-middle" data-title="伝票番号">
                                @if ($order->closing_at != null )
                                    <div class="btn-success">締処理済</div>
                                @endif
                                <a href="{{ route('sale.orders.edit', $order->id) }}">
                                    {{ $order['order_number_zero_fill'] }}
                                </a>
                            </td>
                            {{-- 伝票種別 --}}
                            <td class="text-center" data-title="伝票種別">
                                {{ TransactionType::asselectArray()[$order->transaction_type_id] }}
                            </td>
                            <td class="text-center align-middle" data-title="伝票日付">
                                {{ $order->order_date_slash }}
                            </td>
                            <td class="text-center align-middle" data-title="請求日付">
                                {{ $order->billing_date_slash }}
                            </td>
                            <td class="text-left align-middle" data-title="得意先名">
                                {{ $order->mCustomer->name }}
                            </td>
                            <td class="text-left align-middle" data-title="支所名">
                                {{ $order->mBranch->name ?? null }}
                            </td>
                            <td class="text-left align-middle" data-title="納品先名">
                                {{ $order->mRecipient->name ?? null }}
                            </td>
                            {{-- 部門 --}}
                            <td class="text-left" data-title="部門">
                                {{ $order->department_name }}
                            </td>
                            {{-- 事業所 --}}
                            <td class="text-left" data-title="事業所">
                                {{ $order->office_facilities_name }}
                            </td>
                            <td class="text-right align-middle" data-title="金額計(小計)">
                                {{ number_format($order->sales_total) }}
                            </td>
                            <td class="text-right align-middle" data-title="消費税">
                                @if( $order->tax_calc_type_id === TaxCalcType::ORDER ||
                                    $order->tax_calc_type_id === TaxCalcType::DETAIL)
                                    {{ number_format($order->tax) }}
                                @endif
                                @if( $order->tax_calc_type_id === TaxCalcType::BILLING)
                                    <b>※請求計算</b>
                                @endif
                            </td>
                        </tr>
                        <tr class="detail" style="display: none;">
                            <td colspan="12">
                                <table class="table table-bordered table-responsive-org table-fixed mb-1"
                                       id="order_products_table"
                                       style="max-height: none !important;">
                                    <thead class="thead-light text-center">
                                    <tr class="d-none d-md-table-row">
                                        <th style="width: 3%;">No.</th>
                                        <th style="width: 36%;">商品</th>
                                        <th style="width: 7%;">数量</th>
                                        <th style="width: 5%;">単位</th>
                                        <th style="width: 7%;">単価</th>
                                        <th style="width: 10%;">金額</th>
                                        <th style="width: 8%;">消費税</th>
                                        <th style="width: 13%;">備考</th>
                                    </tr>
                                    </thead>
                                    <tbody>
                                    @foreach ($order->salesOrderDetail as $key => $detail)
                                        <tr>
                                            <th class="text-center">
                                                {{ ++$key }}
                                            </th>
                                            <td class="text-left" data-title="商品">
                                                {{ $detail->product_name }}
                                            </td>
                                            <td class="text-right" data-title="数量">
                                                {{ number_format($detail->quantity) }}
                                            </td>
                                            <td class="text-right" data-title="単位">
                                                {{ $detail->unit_name }}
                                            </td>
                                            <td class="text-right" data-title="単価">
                                                {{ number_format($detail->unit_price) }}
                                            </td>
                                            <td class="text-right" data-title="金額">
                                                {{ number_format($detail->sub_total) }}
                                            </td>
                                            <td class="text-right" data-title="消費税">
                                                @if( $order->tax_calc_type_id === TaxCalcType::DETAIL)
                                                    @if( $detail->consumption_tax_rate === 0)
                                                        非課税
                                                    @else
                                                        @if( $detail->tax_type_id === TaxType::IN_TAX)
                                                            内({{ number_format($detail->sub_total_tax) ?? '' }})
                                                        @endif
                                                        @if( $detail->tax_type_id === TaxType::OUT_TAX)
                                                            {{ number_format($detail->sub_total_tax) }}
                                                        @endif
                                                    @endif
                                                @endif
                                                @if( $order->tax_calc_type_id === TaxCalcType::ORDER)
                                                    <b>※伝票計算</b>
                                                @endif
                                                @if( $order->tax_calc_type_id === TaxCalcType::BILLING)
                                                    <b>※請求計算</b>
                                                @endif
                                            </td>
                                            <td class="text-left" data-title="備考">
                                                {{ $detail->note }}
                                            </td>
                                        </tr>
                                    @endforeach
                                    </tbody>
                                </table>
                            </td>
                        </tr>
                    @endforeach
                    </tbody>
                </table>
            </div>
            {{ $search_result['sales_orders']->appends($search_condition_input_data)->links() }}
        </div>
    </div>

    {{-- JS読み込み --}}
    <script src="{{ mix('js/app/sale/index.js') }}"></script>
    <script src="{{ mix('js/app/sale/orders/index.js') }}"></script>
@endsection
