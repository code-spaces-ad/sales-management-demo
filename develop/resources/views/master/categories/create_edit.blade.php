...
@php
    $headline = config('consts.title.master.menu.categories.create');
    $next_url = route('master.categories.store');
    $next_btn_text = '登録';
    $method = 'POST';
    $is_edit_route = false;
    if (Route::currentRouteName() === 'master.categories.edit') {
        $headline = config('consts.title.master.menu.categories.edit');
        $next_url = route('master.categories.update', $target_record_data['id']);
        $next_btn_text = '更新';
        $method = 'PUT';
        $is_edit_route = true;
    }
...
@section('content')
                ...
                @csrf
                    ...
                    <div class="form-group row my-1">
                        ...
                        <button id="code-spinner" type="button" class="btn btn-primary"
                                onclick="searchAvailableNumber('categories')">
                    ...
                    <div class="buttons-area text-center mt-4">
                        ...
                        href="{{ session($session_master_key, route('master.categories.index')) }}">
                            一覧画面へ戻る
                        </a>
    ...
    <div>
        ...
        <input type="hidden" name="api_get_next_usable_code_url"
            value="{{ route('api.common.get_next_usable_code') }}" class="hidden-api-get-next-usable-code-url">
    </div>
    ...
    @if ($is_edit_route)
        <form name="deleteForm" id="deleteForm" action="{{ route('master.categories.destroy', $target_record_data['id']) }}" method="POST">
        @method('DELETE')
        @csrf
        </form>
    @endif
    ...
    <script src="{{ mix('js/app/master/categories/create_edit.js') }}"></script>
