...
@php
    ...
    $headline = config('consts.title.master.menu.office_facilities.index');
    $next_url = route('master.office_facilities.index');
    $method = 'GET';
@endphp
...
@section('content')
        ...
        <form name="searchForm" action="{{ $next_url }}" method="{{ $method }}" class="col-12 px-0 px-md-4 pb-md-2">
            ...
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    {{-- 事業所名 --}}
                    @include('common.master.index.office_facility_name')
                </div>
        ...
        <div class="col-md-12 d-flex justify-content-between mb-2">
            <div class="download-area">
                ...
                @csrf
                    ...
                    <input type="hidden" name="department_id_code" value="{{ $search_condition_input_data['department_id_code'] ?? '' }}">
                    <input type="hidden" name="name" value="{{ $search_condition_input_data['name'] ?? '' }}">
                </form>

                {{-- Excelダウンロードボタン --}}
                <a class="btn btn-success"
                   onclick="downloadPost('{{ route('master.office_facilities.download_excel')}}');">
        ...
        <div class="col-md-12">
            <div class="form-group d-md-inline-flex col-12 px-0 m-0">
                <div class="form-group d-md-inline-flex col-md-6 p-0 m-0">
                    {{ $search_result['office_facilities']->appends($search_condition_input_data)->links() }}
                </div>
                <div class="col-md-6 m-0 p-0 text-right">
                    <a class="btn btn-primary" href="{{ route('master.office_facilities.create') }}">
            ...
            <div class="result-table-area table-responsive table-fixed" style="max-height: none !important;">
                <table class="table table-bordered table-responsive-org table-list">
                    ...
                    <tr class="text-center">
                        ...
                        <th scope="col" class="col-md-4">事業所名</th>
                        <th scope="col" class="col-md-3">担当者</th>
                        <th scope="col" class="col-md-4">備考</th>
                    </tr>
                    </thead>
                    <tbody>
                    @foreach ($search_result['office_facilities'] ?? [] as $key => $office_facility)
                        <tr>
                            <th scope="row" class="text-center align-middle" onclick="checkingClickedOrNot('master.office_facilities');"
                                data-title="編集">
                                <a href="{{ route('master.office_facilities.edit', $office_facility->id) }}">
                                    <label class="btn btn-outline-info m-0">編集</label>
                                </a>
                            </th>
                            <td class="text-center align-middle" data-title="コード">
                                {{  $office_facility->code_zerofill }}
                            </td>
                            <td class="text-left align-middle" data-title="事業所名">
                                <div style="font-size: 0.8rem;">{{StringHelper::getNameWithId($office_facility->mDepartment->code_zerofill,$office_facility->mDepartment->name)}}</div>
                                <div style="font-size: 1.0rem;font-weight: bold">┗{{$office_facility->name}}</div>
                            </td>
                            <td class="text-left align-middle" data-title="担当者">
                                {{ $office_facility->employee_name ?? '' }}
                            </td>
                            <td class="text-left align-middle" data-title="備考">
                                {{ $office_facility->note }}
                            </td>
                        </tr>
                    @endforeach
                    </tbody>
    ...
    {{-- JS読み込み --}}
    <script src="{{ mix('js/app/master/office_facilities/index.js') }}"></script>
@endsection
