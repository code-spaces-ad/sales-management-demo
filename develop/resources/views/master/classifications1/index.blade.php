...
@php
    ...
    $headline = config('consts.title.master.menu.classifications1.index');
    $next_url = route('master.classifications1.index');
    $method = 'GET';
@endphp
...
@section('content')
        ...
        <form name="searchForm" action="{{ $next_url }}" method="{{ $method }}" class="col-12 px-0 px-md-4 pb-md-2">
            ...
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    {{-- 分類1名 --}}
                    @include('common.master.index.classification1_name')
                </div>
        ...
        <div class="col-md-12 d-flex justify-content-between mb-2">
            <div class="download-area">
                ...
                <a class="btn btn-success"
                   onclick="downloadPost('{{ route('master.classifications1.download_excel')}}');">
        ...
        <div class="col-md-12">
            <div class="form-group d-md-inline-flex col-12 px-0 m-0">
                <div class="form-group d-md-inline-flex col-md-6 p-0 m-0">
                    {{ $search_result['classifications1']->appends($search_condition_input_data)->links() }}
                </div>
                <div class="col-md-6 m-0 p-0 text-right">
                    <a class="btn btn-primary" href="{{ route('master.classifications1.create') }}">
            ...
            <div class="result-table-area table-responsive table-fixed" style="max-height: none !important;">
                <table class="table table-bordered table-responsive-org table-list">
                    ...
                    <tr class="text-center">
                        ...
                        <th scope="col" class="col-md-11">分類1名</th>
                    </tr>
                    </thead>
                    <tbody>
                    @foreach ($search_result['classifications1'] ?? [] as $key => $classification1)
                        <tr>
                            <th scope="row" class="text-center align-middle" onclick="checkingClickedOrNot('master.classifications1');"
                                data-title="編集">
                                <a href="{{ route('master.classifications1.edit', $classification1->id) }}">
                                    <label class="btn btn-outline-info m-0">編集</label>
                                </a>
                            </th>
                            <td class="text-center align-middle" data-title="コード">
                                {{ $classification1->code_zerofill }}
                            </td>
                            <td class="text-left align-middle" data-title="分類1名">
                                <div style="font-size: 1.0rem;font-weight: bold">{{ $classification1->name }}</div>
                            </td>
                        </tr>
                    @endforeach
                    </tbody>
    ...
    {{-- JS読み込み --}}
    <script src="{{ mix('js/app/master/classifications1/index.js') }}"></script>
@endsection
