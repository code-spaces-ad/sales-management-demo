...
@php
    $headline = config('consts.title.master.menu.sub_categories.create');
    $next_url = route('master.sub_categories.store');
    $next_btn_text = '登録';
    $method = 'POST';
    $is_edit_route = false;
    if ((Route::currentRouteName() === 'master.sub_categories.edit')) {
        $headline = config('consts.title.master.menu.sub_categories.edit');
        $next_url = route('master.sub_categories.update', $target_record_data['id']);
        $next_btn_text = '更新';
        $method = 'PUT';
        $is_edit_route = true;
    }
...
@section('content')
        ...
        <div class="col-md-12">
            <div class="input-area">
                ...
                @csrf
                    ...
                    {{-- カテゴリー --}}
                    @include('master.common.create_edit.category_select_list', ['required_category' => true])
                    ...
                    <div class="form-group row my-1">
                        ...
                        <div class="col-sm-2">
                            <input type="text" name="code" id="code"
                                ...
                                class="form-control input-code {{ $errors->has('code') ? 'is-invalid' : '' }}" maxlength="{{ MasterSubCategoriesConst::CODE_MAX_LENGTH }}">
                        ...
                        <button id="code-spinner" type="button" class="btn btn-primary"
                                onclick="searchAvailableNumber('sub_categories', 'category_id', $('.input-category-select').val())">
                    ...
                    <div class="buttons-area text-center mt-4">
                        ...
                        href="{{ session($session_master_key, route('master.sub_categories.index')) }}">
                            一覧画面へ戻る
                        </a>
    ...
    @if ($is_edit_route)
        <form name="deleteForm" id="deleteForm" action="{{ route('master.sub_categories.destroy', $target_record_data['id']) }}" method="POST">
        @method('DELETE')
        @csrf
        </form>
    @endif
    ...
    <script src="{{ mix('js/app/master/sub_categories/create_edit.js') }}"></script>
