...
@php
    $headline = config('consts.title.master.menu.warehouses.create');
    $next_url = route('master.warehouses.store');
    $next_btn_text = '登録';
    $method = 'POST';
    $is_edit_route = false;
    if ((Route::currentRouteName() === 'master.warehouses.edit')) {
        $headline = config('consts.title.master.menu.warehouses.edit');
        $next_url = route('master.warehouses.update', $target_record_data['id']);
        $next_btn_text = '更新';
        $method = 'PUT';
        $is_edit_route = true;
    }
...
@section('content')
                ...
                @csrf
                    ...
                    <div class="form-group row my-1">
                        ...
                        <div class="col-sm-2">
                            <input type="text" name="code" id="code"
                                ...
                                class="form-control input-code {{ $errors->has('code') ? 'is-invalid' : '' }}" maxlength="{{ MasterWarehousesConst::CODE_MAX_LENGTH }}">
                        ...
                        <button id="code-spinner" type="button" class="btn btn-primary"
                                onclick="searchAvailableNumber('warehouses')">
                    ...
                    <div class="form-group row my-3">
                        ...
                        <div class="col-sm-6 col-6">
                            @foreach (($input_items['is_control_inventory'] ?? []) as $key => $val)
                                <div class="icheck-primary icheck-inline mr-2">
                                    <input type="radio" name="is_control_inventory" value="{{ $key }}" id="is-control-inventory-{{ $key }}"
                                        {{ old('is_control_inventory', $target_record_data['is_control_inventory'] ?? 0) == $key ? 'checked' : '' }}>
                                    <label class="form-check-label"
                                        for="is-control-inventory-{{ $key }}">{{ $val }}</label>
                                </div>
                            @endforeach
                            @error('is_control_inventory')
                            <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                    </div>
                    ...
                    <div class="buttons-area text-center mt-4">
                        ...
                        href="{{ session($session_master_key, route('master.warehouses.index')) }}">
                            一覧画面へ戻る
                        </a>
    ...
    @if ($is_edit_route)
        <form name="deleteForm" id="deleteForm" action="{{ route('master.warehouses.destroy', $target_record_data['id']) }}" method="POST">
        @method('DELETE')
        @csrf
        </form>
    @endif
    ...
    <script src="{{ mix('js/app/master/warehouses/create_edit.js') }}"></script>
@endsection
