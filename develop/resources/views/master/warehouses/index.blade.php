...
@php
    ...
    $headline = config('consts.title.master.menu.warehouses.index');
    $next_url = route('master.warehouses.index');
    $method = 'GET';
@endphp
...
@section('content')
        ...
        <form name="searchForm" action="{{ $next_url }}" method="{{ $method }}" class="col-12 px-0 px-md-4 pb-md-2">
            ...
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    {{-- 倉庫名 --}}
                    @include('common.master.index.warehouse_name')
                </div>
        ...
        <div class="col-md-12 d-flex justify-content-between mb-2">
            <div class="download-area">
                ...
                <a class="btn btn-success"
                   onclick="downloadPost('{{ route('master.warehouses.download_excel')}}');">
        ...
        <div class="col-md-12">
            <div class="form-group d-md-inline-flex col-12 px-0 m-0">
                <div class="form-group d-md-inline-flex col-md-6 p-0 m-0">
                    {{ $search_result['warehouses']->appends($search_condition_input_data)->links() }}
                </div>
                <div class="col-md-6 m-0 p-0 text-right">
                    <a class="btn btn-primary" href="{{ route('master.warehouses.create') }}">
            ...
            <div class="result-table-area table-responsive table-fixed" style="max-height: none !important;">
                <table class="table table-bordered table-responsive-org table-list">
                    ...
                    <tr class="text-center">
                        ...
                        <th scope="col" class="col-md-10">倉庫名</th>
                        <th scope="col" class="col-md-1">在庫管理</th>
                    </tr>
                    </thead>
                    <tbody>
                    @foreach ($search_result['warehouses'] ?? [] as $key => $warehouse)
                        <tr>
                            <th scope="row" class="text-center align-middle" onclick="checkingClickedOrNot('master.warehouses');"
                                data-title="編集">
                            ...
                            <td class="text-left align-middle" data-title="倉庫名">
                                <div style="font-size: 0.6rem;">{{ $warehouse->name_kana }}&nbsp;</div>
                                <div style="font-size: 1.0rem;font-weight: bold">{{ $warehouse->name }}</div>
                            </td>
                            <td class="text-left align-middle" data-title="在庫管理">
                                {{ IsControlInventory::getDescription($warehouse->is_control_inventory) }}
                            </td>
                        </tr>
                    @endforeach
                    </tbody>
    ...
    {{-- JS読み込み --}}
    <script src="{{ mix('js/app/master/warehouses/index.js') }}"></script>
@endsection
