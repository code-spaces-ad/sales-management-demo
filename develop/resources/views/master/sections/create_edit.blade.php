...
@php
    $headline = config('consts.title.master.menu.sections.create');
    $next_url = route('master.sections.store');
    $next_btn_text = '登録';
    $method = 'POST';
    $is_edit_route = false;
    if ((Route::currentRouteName() === 'master.sections.edit')) {
        $headline = config('consts.title.master.menu.sections.edit');
        $next_url = route('master.sections.update', $target_record_data['id']);
        $next_btn_text = '更新';
        $method = 'PUT';
        $is_edit_route = true;
    }
...
@section('content')
            ...
            <div class="input-area">
                ...
                @csrf
                    ...
                    <div class="form-group row my-1">
                        ...
                        <div class="col-sm-2">
                            <input type="text" name="code" id="code"
                                ...
                                class="form-control input-code {{ $errors->has('code') ? 'is-invalid' : '' }}" maxlength="{{ MasterSectionsConst::CODE_MAX_LENGTH }}">
                        ...
                        <button id="code-spinner" type="button" class="btn btn-primary"
                                onclick="searchAvailableNumber('sections')">
                    ...
                    <div class="buttons-area text-center mt-4">
                        ...
                        href="{{ session($session_master_key, route('master.sections.index')) }}">
                            一覧画面へ戻る
                        </a>
    ...
    @if ($is_edit_route)
        <form name="deleteForm" id="deleteForm" action="{{ route('master.sections.destroy', $target_record_data['id']) }}" method="POST">
        @method('DELETE')
        @csrf
        </form>
    @endif
    ...
    <script src="{{ mix('js/app/master/sections/create_edit.js') }}"></script>
