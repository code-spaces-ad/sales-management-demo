...
@php
    ...
    $headline = config('consts.title.master.menu.accounting_codes.index');
    $next_url = route('master.accounting_codes.index');
    $method = 'GET';
@endphp
...
@section('content')
        ...
        <form name="searchForm" action="{{ $next_url }}" method="{{ $method }}" class="col-12 px-0 px-md-4 pb-md-2">
            ...
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    {{-- 種別名 --}}
                    @include('common.master.index.accounting_code_name')
                </div>
        ...
        <div class="col-md-12 d-flex justify-content-between mb-2">
            <div class="download-area">
                ...
                <a class="btn btn-success"
                   onclick="downloadPost('{{ route('master.accounting_codes.download_excel')}}');">
        ...
        <div class="col-md-12">
            <div class="form-group d-md-inline-flex col-12 px-0 m-0">
                <div class="form-group d-md-inline-flex col-md-6 p-0 m-0">
                    {{ $search_result['accounting_codes']->appends($search_condition_input_data)->links() }}
                </div>
                <div class="col-md-6 m-0 p-0 text-right">
                    <a class="btn btn-primary" href="{{ route('master.accounting_codes.create') }}">
            ...
            <div class="result-table-area table-responsive table-fixed" style="max-height: none !important;">
                <table class="table table-bordered table-responsive-org table-list">
                    ...
                    <tr class="text-center">
                        ...
                        <th scope="col" class="col-md-3">経理コード名</th>
                        <th scope="col" class="col-md-6">備考</th>
                        <th scope="col" class="col-md-2">出力対象</th>
                    </tr>
                    </thead>
                    <tbody>
                    @foreach ($search_result['accounting_codes'] ?? [] as $key => $accounting_code)
                        <tr>
                            <th scope="row" class="text-center align-middle" onclick="checkingClickedOrNot('master.accounting_codes');"
                                data-title="編集">
                                <a href="{{ route('master.accounting_codes.edit', $accounting_code->id) }}">
                                    <label class="btn btn-outline-info m-0">編集</label>
                                </a>
                            </th>
                            <td class="text-center align-middle" data-title="コード">
                                {{ $accounting_code->code_zerofill }}
                            </td>
                            <td class="text-left align-middle" data-title="経理コード名">
                                {{ $accounting_code->name }}
                            </td>
                            <td class="text-left align-middle" data-title="備考">
                                {{ $accounting_code->note }}
                            </td>
                            <td class="text-left align-middle" data-title="出力対象">
                                {{ $accounting_code->output_group }}
                            </td>
                        </tr>
                    @endforeach
                    </tbody>
    ...
    {{-- JS読み込み --}}
    <script src="{{ mix('js/app/master/accounting_codes/index.js') }}"></script>
@endsection
