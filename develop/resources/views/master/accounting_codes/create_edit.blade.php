...
@php
    $headline = config('consts.title.master.menu.accounting_codes.create');
    $next_url = route('master.accounting_codes.store');
    $next_btn_text = '登録';
    $method = 'POST';
    $is_edit_route = false;
    if ((Route::currentRouteName() === 'master.accounting_codes.edit')) {
        $headline = config('consts.title.master.menu.accounting_codes.edit');
        $next_url = route('master.accounting_codes.update', $target_record_data['id']);
        $next_btn_text = '更新';
        $method = 'PUT';
        $is_edit_route = true;
    }
...
@section('content')
                ...
                @csrf
                    ...
                    <div class="form-group row my-1">
                        ...
                        <button id="code-spinner" type="button" class="btn btn-primary"
                                onclick="searchAvailableNumber('accounting_codes')">
                    ...
                    <div class="form-group row my-3">
                        ...
                        <div class="col-sm-2">
                            <input type="number" name="output_group"
                                   value="{{ old('output_group', $target_record_data['output_group'] ?? 0) }}"
                                   class="form-control input-output_group{{ $errors->has('output_group') ? ' is-invalid' : '' }}">
                    ...
                    <div class="buttons-area text-center mt-4">
                        ...
                        href="{{ session($session_master_key, route('master.accounting_codes.index')) }}">
                            一覧画面へ戻る
                        </a>
    ...
    @if ($is_edit_route)
        <form name="deleteForm" id="deleteForm" action="{{ route('master.accounting_codes.destroy', $target_record_data['id']) }}" method="POST">
        @method('DELETE')
        @csrf
        </form>
    @endif
    ...
    <script src="{{ mix('js/app/master/accounting_codes/create_edit.js') }}"></script>
