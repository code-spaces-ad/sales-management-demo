...
@php
    ...
    $headline = config('consts.title.master.menu.kinds.index');
    $next_url = route('master.kinds.index');
    $method = 'GET';
@endphp
...
@section('content')
        ...
        <form name="searchForm" action="{{ $next_url }}" method="{{ $method }}" class="col-12 px-0 px-md-4 pb-md-2">
            ...
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    {{-- 種別名 --}}
                    @include('common.master.index.kind_name')
                </div>
        ...
        <div class="col-md-12 d-flex justify-content-between mb-2">
            <div class="download-area">
                ...
                <a class="btn btn-success"
                   onclick="downloadPost('{{ route('master.kinds.download_excel')}}');">
        ...
        <div class="col-md-12">
            <div class="form-group d-md-inline-flex col-12 px-0 m-0">
                <div class="form-group d-md-inline-flex col-md-6 p-0 m-0">
                    {{ $search_result['kinds']->appends($search_condition_input_data)->links() }}
                </div>
                <div class="col-md-6 m-0 p-0 text-right">
                    <a class="btn btn-primary" href="{{ route('master.kinds.create') }}">
            ...
            <div class="result-table-area table-responsive table-fixed" style="max-height: none !important;">
                <table class="table table-bordered table-responsive-org table-list">
                    ...
                    <tr class="text-center">
                        ...
                        <th scope="col" class="col-md-11">種別名</th>
                    </tr>
                    </thead>
                    <tbody>
                    @foreach ($search_result['kinds'] ?? [] as $key => $kind)
                        <tr>
                            <th scope="row" class="text-center align-middle" onclick="checkingClickedOrNot('master.kinds');"
                                data-title="編集">
                                <a href="{{ route('master.kinds.edit', $kind->id) }}">
                            ...
                            <td class="text-left align-middle" data-title="種別名">
                                <div style="font-size: 1.0rem;font-weight: bold">{{ $kind->name }}</div>
                            </td>
                        </tr>
                    @endforeach
                    </tbody>
    ...
    {{-- JS読み込み --}}
    <script src="{{ mix('js/app/master/kinds/index.js') }}"></script>
@endsection
