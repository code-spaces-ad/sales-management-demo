...
@php
    $headline = config('consts.title.master.menu.customers.index');
    $next_url = route('master.customers.index');
    $method = 'GET';
@endphp
...
@section('content')
        ...
        <form name="searchForm" action="{{ $next_url }}" method="{{ $method }}" class="col-12 px-0 px-md-4 pb-md-2">
            ...
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    {{-- 得意先名 --}}
                    @include('common.master.index.customer_name')

                    {{-- 請求締日 --}}
                    @include('common.master.index.customer_closing_date')
                </div>
        ...
        <div class="col-md-12 d-flex justify-content-between mb-2">
            <div class="download-area">
                ...
                <a class="btn btn-success"
                   onclick="downloadPost('{{ route('master.customers.download_excel')}}');">
        ...
        <div class="col-md-12">
            <div class="form-group d-md-inline-flex col-12 px-0 m-0">
                <div class="form-group d-md-inline-flex col-md-6 p-0 m-0">
                    {{ $search_result['customers']->appends($search_condition_input_data)->links() }}
                </div>
                <div class="col-md-6 m-0 p-0 text-right">
                    <a class="btn btn-primary" href="{{ route('master.customers.create') }}">
            ...
            <div class="result-table-area table-responsive table-fixed" style="max-height: none !important;">
                <table class="table table-bordered table-responsive-org table-list">
                    ...
                    <tr class="text-center">
                        ...
                        <th scope="col" class="col-md-1">請求締日</th>
                        <th scope="col" class="col-md-1">ソート番号</th>
                    </tr>
                    </thead>
                    <tbody>
                    @foreach ($search_result['customers'] ?? [] as $key => $customer)
                        <tr>
                            <th scope="row" class="text-center align-middle" onclick="checkingClickedOrNot('master.customers');"
                                data-title="編集">
                                <a href="{{ route('master.customers.edit', $customer->id) }}">
                            ...
                            <td class="text-left align-middle" data-title="得意先名">
                                <div style="font-size: 0.6rem;">{{ $customer->name_kana }}&nbsp;</div>
                                <div style="font-size: 1.0rem;font-weight: bold">{{ $customer->name }}</div>
                            </td>
                            <td class="text-left align-middle" data-title="住所">
                                <div>
                                    @if ( !is_null($customer->postal_code1))
                                        〒{{ $customer->postal_code1.'-'.$customer->postal_code2 }}
                                    @else
                                        &nbsp;
                                    @endif
                                </div>
                                <div>
                                    ...
                                    @if ( !is_null($customer->address1))
                                        <a href="{{config('consts.default.api.google_address_search').($customer->address1 ?? '').($customer->address2 ?? '')}}"
                                           target="_blank">
                            ...
                            <td class="text-center align-middle" data-title="税区分">
                                {{ \App\Enums\TaxCalcType::asSelectArray()[$customer->tax_calc_type_id]}}
                            </td>
                            <td class="text-left align-middle" data-title="請求締日">
                                @if( $customer->tax_calc_type_id === \App\Enums\TaxCalcType::NONE )
                                    -
                                @else
                                    {{ config('consts.default.common.closing_date_list')[$customer->closing_date] }}日締め
                                    @if ( $customer->billing_customer_id !== null && $customer->id !== $customer->billing_customer_id )
                                        別途請求先：{{$customer->mBillingCustomer->name}}
                                    @else
                                        &nbsp;
                                    @endif
                                @endif
                            </td>
                            <td class="text-center align-middle" data-title="ソート番号">
                                {{$customer->sort_code}}
                            </td>
                        </tr>
                    @endforeach
    ...
    {{-- JS読み込み --}}
    <script src="{{ mix('js/app/master/customers/index.js') }}"></script>
@endsection
