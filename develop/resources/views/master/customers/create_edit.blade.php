...
@php
    $headline = config('consts.title.master.menu.customers.create');
    $next_url = route('master.customers.store');
    $next_btn_text = '登録';
    $method = 'POST';
    $is_edit_route = false;
    if ((Route::currentRouteName() === 'master.customers.edit')) {
        $headline = config('consts.title.master.menu.customers.edit');
        $next_url = route('master.customers.update', $target_record_data['id']);
        $next_btn_text = '更新';
        $method = 'PUT';
        $is_edit_route = true;
    }

    $honorific_title_id = $target_record_data['mCustomerHonorificTitle']['honorific_title_id'] ?? null;  // 敬称ID
...
@section('content')
                ...
                @csrf
                    ...
                    <div class="form-group row my-1">
                        ...
                        <div class="col-md-2">
                            <input type="text" name="code"
                                ...
                                class="form-control input-code{{ $errors->has('code') ? ' is-invalid' : '' }}" maxlength="{{ $maxlength_customer_code }}">
                            @error('code')
                            <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        <button id="code-spinner" type="button" class="btn btn-primary"
                                onclick="searchAvailableNumber('customers')">
                    ...
                    <div class="form-group row my-3">
                        ...
                        <div class="col-sm-2 col-5">
                            <select name="honorific_title_id"
                                    class="custom-select input-honorific_title_id @if($errors->has('honorific_title_id')) is-invalid @endif">
                                @foreach (($input_items['honorific_titles'] ?? []) as $item)
                                    <option
                                        @if ($item['id'] == old('honorific_title_id', $honorific_title_id)) selected @endif
                                    value="{{ $item['id'] }}">
                                        {{ $item['name_with_id'] }}
                                    </option>
                                @endforeach
                            </select>
                            @error('honorific_title_id')
                            <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                    </div>
                    ...
                    <div class="form-group row my-3">
                        ...
                        <div class="col-sm-5">
                            <input type="text" name="address1"
                                value="{{ old('address1', $target_record_data['address1'] ?? '') }}"
                    ...
                    <div class="form-group row my-3">
                        ...
                        <div class="col-sm-5">
                            <input type="text" name="address2"
                                value="{{ old('address2', $target_record_data['address2'] ?? '') }}"
                    ...
                    <div class="form-group row my-3">
                        ...
                        <div class="col-sm-2">
                            <input type="text" name="tel_number"
                                value="{{ old('tel_number', $target_record_data['tel_number'] ?? '') }}"
                    ...
                    <div class="form-group row my-3">
                        ...
                        <div class="col-sm-2">
                            <input type="text" name="fax_number"
                                value="{{ old('fax_number', $target_record_data['fax_number'] ?? '') }}"
                    ...
                    <div class="form-group row my-3">
                        ...
                        <div class="col-sm-5">
                            {{-- 集計グループコード --}}
                            <input type="number"
                                   class="form-control input-summary-group-code h-75 col-5 col-md-4 mr-md-1 d-none"
                                   id="summary_group_code" oninput="inputCode(this);"
                                   onchange="changeSummaryGroupCode(this);">
                            {{-- 集計グループ --}}
                            <select name="summary_group_id" onchange="changeSummaryGroup();"
                                    class="custom-select input-summary-group-select col-9 px-0 mr-md-1 select2_search d-none
                                    @if($errors->has('summary_group_id')) is-invalid @endif">
                                <option value="">-----</option>
                                @foreach (($input_items['summary_group'] ?? []) as $item)
                                    <option
                                        @if ($item['id'] == old('summary_group_id', $target_record_data['summary_group_id'] ?? null))
                                        selected
                                        @endif
                                        value="{{ $item['id'] }}"
                                        data-name="{{ $item['name'] }}"
                    ...
                    <div class="form-group row my-3">
                        ...
                        <div class="col-sm-4 col-5">
                            @foreach (($input_items['rounding_methods'] ?? []) as $key => $item)
                                <div class="icheck-primary icheck-inline mr-2">
                                    <input type="radio" name="tax_rounding_method_id" id="tax-rounding-method-id-{{ $key }}" value="{{ $item['id'] }}"
                    ...
                    <div class="form-group row my-3">
                        ...
                        <div class="col-sm-6 col-6">
                            @foreach (($input_items['transaction_types'] ?? []) as $key => $val)
                                <div class="icheck-primary icheck-inline mr-2">
                                    <input type="radio" name="transaction_type_id" id="transaction-type-id-{{ $key }}" value="{{ $key }}"
                                        {{ old('transaction_type_id', $target_record_data['transaction_type_id'] ?? config('consts.default.master.customers.transaction_type')) == $key ? 'checked' : '' }}>
                                    <label class="form-check-label"
                                        for="transaction-type-id-{{ $key }}">{{ $val }}</label>
                                </div>
                            @endforeach
                    ...
                    <div class="form-group row my-3 d-none">
                        ...
                        <div class="col-sm-5">
                            <div class="d-md-inline-flex w-100">
                                {{-- 請求先（得意先）検索モーダル表示ボタン --}}
                                <button type="button"
                                        class="btn-sm btn-outline-info mr-md-1"
                                        data-toggle="modal"
                                        data-target="#search-billing-customer">
                                    <i class="fas fa-search"></i>
                                </button>
                                {{-- 請求先（得意先）コード（セレクトボックス選択用） --}}
                                <input type="number"
                                    class="form-control input-billing-customer-code w-50 mr-md-2"
                                    id="billing_customer_code"
                                    oninput="inputCode(this);"
                                    onchange="changeBillingCustomerCode(this);"
                                    maxlength="{{ $maxlength_customer_code }}"
                                    min="{{ $min_customer_code }}">
                                {{-- 請求先（得意先） --}}
                                <select name="billing_customer_id"
                                        onchange="changeBillingCustomer();"
                                        class="custom-select input-billing-customer-select w-md-50 mr-md-1 @if($errors->has('billing_customer_id')) is-invalid @endif">
                                    <option value="">(得意先と同じ)</option>
                                    @foreach (($input_items['billing_customers'] ?? []) as $item)
                                        <option
                                            @if ($item['id'] == old('billing_customer_id', $target_record_data['billing_customer_id'] ?? null)) selected
                                            @endif
                                            value="{{ $item['id'] }}"
                                            data-code="{{ $item['code'] }}">
                    ...
                    <div class="form-group row my-1">
                        ...
                        <div class="col-md-2">
                            <input type="text" name="sort_code" value="{{ old('sort_code', $target_record_data['sort_code'] ?? '') }}"
                                class="form-control input-sort-code{{ $errors->has('sort_code') ? ' is-invalid' : '' }}" maxlength="{{ $maxlength_customer_code }}">

                            @error('sort_code')
                            <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        <button id="sort-spinner" type="button" class="btn btn-primary"
                                onclick="searchAvailableSortNumber('customers')">
                            <i class="fa-solid fa-magnifying-glass"></i>
                            <div class="spinner-border spinner-border-sm text-light align-middle" role="status"
                                 style="display: none;"></div>
                            利用可能ソート番号
                    ...
                    <div class="form-group row my-3">
                        ...
                        <div class="col-sm-6 col-6">
                            @foreach (($input_items['collection_months'] ?? []) as $key => $val)
                                <input type="radio" name="collection_month" value="{{ $key }}"
                                    {{ $key === old('collection_month',
                                    $target_record_data['collection_month'] ?? config('consts.default.master.customers.collection_month'))
                                    || $key === $target_record_data['collection_month'] ? 'checked' : '' }}>
                        ...
                        <div class="col-sm-2 col-5">
                            <div class="d-flex flex-row">
                                <select name="collection_day"
                                        class="custom-select input-closing-date clear-select
                                        @if($errors->has('collection_day')) is-invalid @endif">
                                    @foreach (($input_items['collection_day_list'] ?? []) as $key => $item)
                                        <option
                                            @if ($key == old('collection_day', $target_record_data['collection_day'] ??
                                                config('consts.default.master.customers.collection_day'))) selected
                                            @endif
                                            value="{{ $key }}">
                                            {{ $item }}
                                        </option>
                                ...
                                <label class="col-form-label w-100 ml-1">日</label>
                            </div>
                    ...
                    <div class="form-group row my-3">
                        ...
                        <div class="col-sm-6 col-6">
                            @foreach (($input_items['collection_methods'] ?? []) as $key => $val)
                                <input type="radio" name="collection_method" value="{{ $key }}"
                                    {{ $key === old('collection_method',
                                    $target_record_data['collection_method'] ?? config('consts.default.master.customers.collection_method'))
                                    || $key === $target_record_data['collection_method'] ? 'checked' : '' }}>
                    ...
                    <div class="form-group row my-3 d-none">
                        ...
                        <div class="col-sm-6 col-6">
                            @foreach (($input_items['sales_invoice_format_types'] ?? []) as $key => $val)
                                <input type="radio" name="sales_invoice_format_type" value="{{ $key }}"
                                    {{ $key === old('collection_method',
                                    $target_record_data['sales_invoice_format_type'] ?? config('consts.default.master.customers.sales_invoice_format_type'))
                                    || $key === $target_record_data['sales_invoice_format_type'] ? 'checked' : '' }}>

                                {{$val}}&nbsp;&nbsp;&nbsp;
                            @endforeach
                            @error('sales_invoice_format_type')
                            <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                    </div>
                    <div class="form-group row my-3 d-none">
                        ...
                        <div class="col-sm-6 col-6">
                            @foreach (($input_items['sales_invoice_printing_methods'] ?? []) as $key => $val)
                                <input type="radio" name="sales_invoice_printing_method" value="{{ $key }}"
                                    {{ $key === old('sales_invoice_printing_method',
                                    $target_record_data['sales_invoice_printing_method'] ?? config('consts.default.master.customers.sales_invoice_printing_method'))
                                    || $key === $target_record_data['sales_invoice_printing_method'] ? 'checked' : '' }}>

                                {{$val}}&nbsp;&nbsp;&nbsp;
                            @endforeach
                            @error('sales_invoice_printing_method')
                            <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                    </div>
                    ...
                    <div class="form-group row my-3">
                        ...
                        <div class="col-sm-5">
                            <textarea name="note" class="form-control input-note{{ $errors->has('note') ? ' is-invalid' : '' }}" rows="4">{{ old('note', $target_record_data['note'] ?? '') }}</textarea>
                            @error('note')
                            <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                    ...
                    <div class="buttons-area text-center mt-4">
                        ...
                        href="{{ session($session_master_key, route('master.customers.index')) }}">
                            一覧画面へ戻る
                        </a>
    ...
    <div>
        ...
        <input type="hidden" name="api_get_next_usable_sort_code_url"
            value="{{ route('api.common.get_next_usable_sort_code') }}"
            class="hidden-api-get-next-usable-sort-code-url">
    </div>
    ...
    @if ($is_edit_route)
        <form action="{{ route('master.customers.destroy', $target_record_data['id']) }}" name="deleteForm" id="deleteForm" method="POST">
        @method('DELETE')
        @csrf
        </form>
    @endif
    ...
    @component('components.search_customer_modal')
        @slot('modal_id', 'search-billing-customer')
        @slot('customers', $input_items['billing_customers'])
        @slot('onclick_select_customer', "selectBillingCustomerSearchCustomerModal(this);")
    @endcomponent

    {{-- JS読み込み --}}
    <script src="{{ mix('js/app/master/customers/create_edit.js') }}"></script>
@endsection
