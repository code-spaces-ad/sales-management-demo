...
@php
    ...
    $headline = config('consts.title.master.menu.summary_group.index');
    $next_url = route('master.summary_group.index');
    $method = 'GET';
@endphp
...
@section('content')
        ...
        <form name="searchForm" action="{{ $next_url }}" method="{{ $method }}" class="col-12 px-0 px-md-4 pb-md-2">
            ...
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    {{-- 集計グループ名 --}}
                    @include('common.master.index.summary_group_name')
                </div>
        ...
        <div class="col-md-12 d-flex justify-content-between mb-2">
            <div class="download-area">
                ...
                <a class="btn btn-success"
                   onclick="downloadPost('{{ route('master.summary_group.download_excel')}}');">
        ...
        <div class="col-md-12">
            <div class="form-group d-md-inline-flex col-12 px-0 m-0">
                <div class="form-group d-md-inline-flex col-md-6 p-0 m-0">
                    {{ $search_result['summary_group']->appends($search_condition_input_data)->links() }}
                </div>
                <div class="col-md-6 m-0 p-0 text-right">
                    <a class="btn btn-primary" href="{{ route('master.summary_group.create') }}">
            ...
            <div class="result-table-area table-responsive table-fixed" style="max-height: none !important;">
                <table class="table table-bordered table-responsive-org table-list">
                    ...
                    <tr class="text-center">
                        ...
                        <th scope="col" class="col-md-5">集計グループ名</th>
                        <th scope="col" class="col-md-6">備考</th>
                    </tr>
                    </thead>
                    <tbody>
                    @foreach ($search_result['summary_group'] ?? [] as $key => $summary_group)
                        <tr>
                            <th scope="row" class="text-center align-middle" onclick="checkingClickedOrNot('master.summary_group');"
                                data-title="編集">
                                <a href="{{ route('master.summary_group.edit', $summary_group->id) }}">
                                    <label class="btn btn-outline-info m-0">編集</label>
                                </a>
                            </th>
                            <td class="text-center align-middle" data-title="コード">
                                {{ $summary_group->code_zerofill }}
                            </td>
                            <td class="text-left align-middle" data-title="集計グループ名">
                                <div style="font-size: 1.0rem;font-weight: bold">{{ $summary_group->name }}</div>
                            </td>
    ...
    {{-- JS読み込み --}}
    <script src="{{ mix('js/app/master/summary_group/index.js') }}"></script>
@endsection
