...
@php
    $headline = config('consts.title.master.menu.summary_group.create');
    $next_url = route('master.summary_group.store');
    $next_btn_text = '登録';
    $method = 'POST';
    $is_edit_route = false;
    if ((Route::currentRouteName() === 'master.summary_group.edit')) {
        $headline = config('consts.title.master.menu.summary_group.edit');
        $next_url = route('master.summary_group.update', $target_record_data['id']);
        $next_btn_text = '更新';
        $method = 'PUT';
        $is_edit_route = true;
    }
...
@section('content')
                ...
                @csrf
                    ...
                    <div class="form-group row my-1">
                        ...
                        <div class="col-sm-2">
                            <input type="text" name="code" id="code"
                                ...
                                class="form-control input-code {{ $errors->has('code') ? 'is-invalid' : '' }}"
                                   maxlength="{{ MasterSummaryGroupConst::CODE_MAX_LENGTH }}">
                        ...
                        <button id="code-spinner" type="button" class="btn btn-primary"
                                onclick="searchAvailableNumber('summary_group')">
                    ...
                    <div class="form-group row my-3">
                        ...
                        <div class="col-sm-6">
                            <textarea name="note" class="form-control input-note{{ $errors->has('note') ? ' is-invalid' : '' }}"
                                      id="note" rows="5">{{ old('note', $target_record_data['note'] ?? null) }}</textarea>
                            @error('note')
                            <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                    ...
                    <div class="buttons-area text-center mt-4">
                        ...
                        href="{{ session($session_master_key, route('master.summary_group.index')) }}">
                            一覧画面へ戻る
                        </a>
    ...
    @if ($is_edit_route)
        <form name="deleteForm" id="deleteForm" action="{{ route('master.summary_group.destroy', $target_record_data['id']) }}" method="POST">
        @method('DELETE')
        @csrf
        </form>
    @endif
    ...
    <script src="{{ mix('js/app/master/summary_group/create_edit.js') }}"></script>
