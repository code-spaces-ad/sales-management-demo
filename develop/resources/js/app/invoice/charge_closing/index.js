...
window.chargeClosingStore = function (route) {
    ...
    // チェックボックスがONの一覧取得
    let customer_ids = $('.input-target-charge-closing:checked').map(function () {
...
window.chargeClosingCancel = function () {
    ...
    // チェックボックスがONの一覧取得
    let charge_data_ids = $('.input-target-charge-closing:checked').map(function () {
        if ($(this).closest('tr').children('td')[1].innerText.trim() === '') {
            return;
        }
    ...
    if (charge_data_ids.length === 0) {
        alert('請求締解除の対象が選択されていません。');
        return;
    }
    ...
    // submit用のhiddenセット
    $('#cancelForm').find('input[name="charge_data_ids"]').val(charge_data_ids);
 ...
 * 請求締処理(個別)
 */
window.chargeClosingStoreSingle = function (customer_id, target_name) {
    if (!confirm('「' + target_name + '」を個別に請求締処理します\r\nよろしいですか。\r\n※対象伝票が無い場合は処理されません。')) {
        return;
    }
    ...
    $('#searchForm').append(
        $('<input>')
            .attr('type', 'hidden')
            .attr('name', 'customer_ids')
            .val([customer_id])
    );
    $('#searchForm').attr('action', $('input[name="charge_closing_store"]').val());
 ...
 * 請求締解除処理(個別)
 */
window.chargeClosingCancelSingle = function (charge_data_id, target_name) {

    let target = $(this);

    if (!confirm('「' + target_name + '」の請求締を個別に解除します\r\nよろしいですか。')) {
        return;
    }
