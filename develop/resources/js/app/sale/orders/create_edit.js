...
window.onload = function () {
    ...
    // 支所セレクトボックスフィルタリング
    filterBranch();
    changeCustomerCreateEdit(true);
    changeBranchCreateEdit();
}
...
window.addEventListener('load', function () {
     ...
     * 売上分類変更時処理
     */
    $('#sales_classification, .input-sales-classification-select').on('change', function () {
        const classificationValue = $(this).val();
        const cancelValue = String(window.Laravel.enums.sales_classification.classification_cancel);
        const isCancel = classificationValue === cancelValue;

        $('#order_products_table tbody tr').each(function () {
            const $row = $(this);

            setProductPrice(this);
            setRowColorByClassification($row, classificationValue);
            setProductGross($row);
    ...
    /**
     * 売上分類セレクトボックスから「取消」を選択できないようにする
     */
    $(document).ready(function () {
        const cancelValue = String(window.Laravel.enums.sales_classification.classification_cancel);

        // 「取消」を無効化
        $('#sales_classification option').each(function () {
            if ($(this).val() === cancelValue) {
                $(this).prop('disabled', true);
            }
        });
    ...
    /**
     * 売上分類が「取消」の場合、全ての入力操作を無効化する
     */
    $(document).ready(function () {
        if (String(window.initialSalesClassificationId) === String(window.SALES_CLASSIFICATION_CANCEL)) {
            // 入力無効化
            $('#order_products_table input, #order_products_table select, #order_products_table textarea').prop('disabled', true);

            // 分類セレクトボックス無効化
            $('#sales_classification, .input-sales-classification-select').prop('disabled', true);

            // ボタン無効化
            $('#submit_button, #save_button').prop('disabled', true);
        }
    });
    ...
    $('#order_products_table tbody tr').each(function () {
        ...
        setSalesDiscountNumberFormat();
        // 単価桁区切り処理セット
        setUnitPriceNumberFormat($(this));
        // 仕入単価桁区切り処理セット
        setPurchaseUnitPriceNumberFormat($(this));
        // 金額端数処理セット
     ...
     * 値引額 ブラーイベント
     */
    $(".input-sales-discount").blur(function () {
        let discountStr = $(this).val().replace(/[^\-\.0-9]/g, '');    // ※マイナス値可
        if (discountStr.length === 0) {
            discountStr = '0';
        }
     ...
     * 仕入単価 フォーカスイベント
     */
    $(".input-unit-price-purchase").focus(function () {
        // 桁区切りを一旦解除
        let purchaseUnitPrice = $(this).val().replace(/,/g, '');
        $(this).val(purchaseUnitPrice);
     ...
     * 仕入単価 キーダウンイベント
     */
    $(".input-unit-price-purchase").keydown(function (event) {
        // 「type='number'」は「e」が入力可能なので除外
        if (event.key === 'e') {
            return false;
        }
     ...
     * 仕入単価 ブラーイベント
     */
    $(".input-unit-price-purchase").blur(function () {
        let purchaseUnitPriceStr = $(this).val().replace(/[^\-\.0-9]/g, '');    // ※マイナス値可
        if (purchaseUnitPriceStr.length === 0) {
            purchaseUnitPriceStr = '0';
        }

        let purchaseUnitPrice = parseFloat(purchaseUnitPriceStr);
        let digit = $(this).closest('tr').find('.input-product-select option:selected').data('unit-price-decimal-digit');
        let method = $(this).closest('tr').find('.input-product-select option:selected').data('amount-rounding-method-id');
        let calcUnitPrice = getFloorValueForDigit(purchaseUnitPrice, digit, method);
...
window.clearInput = function () {
    ...
    $('.input-transaction-type-select').prop('selectedIndex', 1);
    $('.input-customer-select').prop('selectedIndex', 0);
    changeCustomerCreateEdit();

    $('.input-billing-date').val($('#default_order_date').val());
    $('.input-branch-select').val('');
    $('.label-tax-type-name').text('[税抜]');
    $('.label-tax-unit-name').text('％');
    $('.input-product-code').val('');
    $('.input-product-select').prop('selectedIndex', 0).change();
    $('.input-product-name').val('');
    $('.input-quantity').val('');
    $('.input-product-unit-name-select').prop('selectedIndex', 0).change();
    $('.input-unit-price').val('');
    $('.input-unit-price-purchase').val('');
    $('.input-product-sub-total').val('0');
    $('.input-product-sub-gross').val('');
    ...
    // 各種税額（登録用の隠し項目）
    $('.hidden-sales-total').val('0');
    $('.hidden-sales-total-normal-out').val('0');
    $('.hidden-sales-total-reduced-out').val('0');
    $('.hidden-sales-total-normal-in').val('0');
    $('.hidden-sales-total-reduced-in').val('0');
    $('.hidden-sales-total-free').val('0');
    $('.hidden-sales-tax-normal-out').val('0');
    $('.hidden-sales-tax-reduced-out').val('0');
    $('.hidden-sales-tax-normal-in').val('0');
    $('.hidden-sales-tax-reduced-in').val('0');

    $('.text-sales-total-normal-out').text('&yen;0');
    $('.text-sales-total-reduced-out').text('&yen;0');
    $('.text-sales-total-normal-in').text('&yen;0');
    $('.text-sales-total-reduced-in').text('&yen;0');
    $('.text-sales-total-free').text('&yen;0');
    $('.text-sales-tax-normal-out').text('&yen;0');
    $('.text-sales-tax-reduced-out').text('&yen;0');
    $('.text-sales-tax-normal-in').text('&yen;0');
    $('.text-sales-tax-reduced-in').text('&yen;0');
...
window.editFormSubmit = function () {
    ...
    $("[class*='input-product-sub-gross']").each(function (index) {
        let amount = $(this).val().replace(/,/g, '');
        $(this).val(amount);
    });
...
window.clearProduct = function (target) {
    ...
    row.find('.input-unit-price-purchase').val('');
    row.find('.input-product-sub-gross').val('');
    ...
    row.find('.label-tax-unit-name').text('％');
...
* 得意先セレクトボックス変更処理
*/
window.changeCustomerCreateEdit = function (flg_onload = false) {
...
window.changeProductCodeCreateEdit = function (target) {
    ...
    if (row.find(targetOption).not('.d-none').length === 1) {
        ...
        // 対象コードがなければ、最初のリストを選択状態にする
        row.find('.input-product-select').prop('selectedIndex', 0);
        row.find('.input-product-code').val('');
    ...
    // 選択された商品の仕入単価をセット
    row.find('.input-unit-price-purchase')
    .val(row.find('.input-product-select option:selected').data('purchase-unit-price'));
    ...
    // 仕入単価桁区切りセット
    setPurchaseUnitPriceNumberFormat(target);
    // 数量小数桁数セット
...
window.setTaxCalcType = function () {
    ...
    $('.hidden-tax-calc-type-id')
        .val($('.input-customer-select option:selected').data('tax-calc-type'));
}
...
window.setTaxRoundingMethod = function () {
    ...
    $('.hidden-tax-rounding-method-id')
        .val($('.input-customer-select option:selected').data('tax-rounding-method'));
}

/**
* 取引種別セット処理
*/
window.setTransactionType = function () {
    // 選択された得意先の取引種別をセット
    $('.input-transaction-type-select')
        .val($('.input-customer-select option:selected').data('transaction-type-id'));
}
 ...
 * 値引桁区切りセット処理
 */
window.setSalesDiscountNumberFormat = function () {
    let discount = $('.input-sales-discount').val().replace(/,/g, '');
    if (discount === '') {
        return;
    }
    ...
    $('.input-sales-discount').val(number);
}
...
* @param target
*/
window.setPurchaseUnitPriceNumberFormat = function (target) {
    let row = $(target).closest('tr');
    ...
    let digit = row.find('.input-product-select option:selected').data('unit-price-decimal-digit');
    let number = parseFloat(purchaseUnitPrice).toLocaleString(undefined, {
        minimumFractionDigits: digit,
        maximumFractionDigits: digit
    });
    row.find('.input-unit-price-purchase').val(number);
}
...
window.setProductPrice = function (target) {
    ...
    // 数量・単価・値引き取得（カンマを除去）
    let quantityStr = row.find('.input-quantity').val().replace(/,/g, '');
    let quantity = parseFloat(quantityStr);
    if (isNaN(quantity)) {
        return;
    }

    let unitPrice = Math.abs(parseFloat(row.find('.input-unit-price').val().replace(/,/g, ''))) || 0;
    let discount = parseFloat(row.find('.input-discount').val().replace(/,/g, '')) || 0;

    // 売上分類の値を取得（返品＝1）
    let classificationValue = $('.input-sales-classification-select').val();
    let is_return = String(window.Laravel.enums.sales_classification.classification_return);

    // 返品の場合は数量をマイナス
    if (classificationValue === is_return) {
    ...
    // 数量欄に反映（カンマ付き）
    row.find('.input-quantity').val(quantity.toLocaleString());
...
window.setProductGross = function (target) {
    ...
    // 粗利セット
    let sub_total_rounding_method = $('.input-customer-select option:selected').data('tax-rounding-method');
    let subgross = 0;
...
window.setTotalAmounts = function () {
    ...
    let taxCalcTypeId = parseInt($('.input-customer-select option:selected').data('tax-calc-type'));
    let groupTotal = [];
    let sales_discount = parseInt($('.input-sales-discount').val().replace(/,/g, ''));
    ...
    $('#order_products_table tbody tr').each(function () {
        ...
        // 粗利計
        let grossprofit = parseFloat($(this).find('.input-unit-price-purchase').val().replace(/,/g, ''));
        if (isNaN(grossprofit)) {
            grossprofit = 0;
        }
        grossTotal += calcAmountRounding(sub_total_rounding_method, (quantity * price - quantity * grossprofit));
    ...
    $('.text-sales-discount').text('¥' + sales_discount.toLocaleString());
    $('.text-sub-total').text('¥' + (subTotal-sales_discount).toLocaleString());

    // 値引を決定
    calcIndividualGroupDiscount(total, individualGroupDiscount, subTotal, sales_discount);
    // 値引を差し引く
    calcIndividualGroupTotalAfterDiscount(total, individualGroupDiscount);
    // 税金の計算
    let method = $('.input-customer-select option:selected').data('tax-rounding-method');
    row_data.forEach((row, index) => {
        ...
        if (row.reducedTaxFlag === window.Laravel.enums.reduced_tax_flag_type.reduced) {
            if (row.taxTypeId === window.Laravel.enums.tax_type.out_tax) {
                ...
                // 値引案分取得
                diff_discount = calcChunkDiscount(sales_discount, price, subTotal);
    ...
    // 合計
    $('.text-inctax-total').text('¥' + (subTotal - sales_discount).toLocaleString());
    if (taxCalcTypeId === window.Laravel.enums.tax_calc_type.order || taxCalcTypeId === window.Laravel.enums.tax_calc_type.detail) {
        $('.text-inctax-total').text(
            '¥' + (subTotal + total['consumptionTaxTotal'] + total['reducedTaxTotal'] - sales_discount).toLocaleString());
    }
    // 粗利合計
    $('.text-gross-total').text('¥' + grossTotal.toLocaleString());

    // 各種税額のセット
    $('.hidden-sales-total').val(subTotal);
    if (reduced_valid) {
        $('.hidden-sales-total-normal-out').val(total['consumptionTotal']);
        $('.hidden-sales-total-reduced-out').val(total['reducedTotal']);
        $('.hidden-sales-total-normal-in').val(total['consumptionInTotal']);
        $('.hidden-sales-total-reduced-in').val(total['reducedInTotal']);
        $('.hidden-sales-total-free').val(total['notaxTotal']);
        $('.hidden-sales-tax-normal-out').val(total['consumptionTaxTotal']);
        $('.hidden-sales-tax-reduced-out').val(total['reducedTaxTotal']);
        $('.hidden-sales-tax-normal-in').val(total['consumptionInTaxTotal']);
        $('.hidden-sales-tax-reduced-in').val(total['reducedInTaxTotal']);
    } else {
        $('.hidden-sales-total-normal-out').val(total['consumptionTotal'] + total['reducedTotal']);
        $('.hidden-sales-total-normal-in').val(total['consumptionInTotal'] + total['reducedInTotal']);
        $('.hidden-sales-total-free').val(total['notaxTotal']);
        $('.hidden-sales-tax-normal-out').val(total['consumptionTaxTotal'] + total['reducedTaxTotal']);
        $('.hidden-sales-tax-normal-in').val(total['consumptionInTaxTotal'] + total['reducedInTaxTotal']);
    ...
    if (reduced_valid) {
        $('.text-sales-total-normal-out').text('¥' + total['consumptionTotal'].toLocaleString());
        $('.text-sales-total-reduced-out').text('¥' + total['reducedTotal'].toLocaleString());
        $('.text-sales-total-normal-in').text('(¥' + total['consumptionInTotal'].toLocaleString() + ')');
        $('.text-sales-total-reduced-in').text('(¥' + total['reducedInTotal'].toLocaleString() + ')');
        $('.text-sales-total-free').text('¥' + total['notaxTotal'].toLocaleString());
        $('.text-sales-tax-normal-out').text('¥' + total['consumptionTaxTotal'].toLocaleString());
        $('.text-sales-tax-reduced-out').text('¥' + total['reducedTaxTotal'].toLocaleString());
        $('.text-sales-tax-normal-in').text('(¥' + total['consumptionInTaxTotal'].toLocaleString() + ')');
        $('.text-sales-tax-reduced-in').text('(¥' + total['reducedInTaxTotal'].toLocaleString() + ')');
    } else {
        $('.text-sales-total-normal-out').text('¥' + (total['consumptionTotal'] + total['reducedTotal']).toLocaleString());
        $('.text-sales-total-normal-in').text('(¥' + (total['consumptionInTotal'] + total['reducedInTotal']).toLocaleString() + ')');
        $('.text-sales-total-free').text('¥' + total['notaxTotal'].toLocaleString());
        $('.text-sales-tax-normal-out').text('¥' + (total['consumptionTaxTotal'] + total['reducedTaxTotal']).toLocaleString());
        $('.text-sales-tax-normal-in').text('(¥' + (total['consumptionInTaxTotal'] + total['reducedInTaxTotal']).toLocaleString() + ')');

        $('.text-sales-total-reduced-out').text('¥0');
        $('.text-sales-total-reduced-in').text('(¥0)');
        $('.text-sales-tax-reduced-out').text('¥0');
        $('.text-sales-tax-reduced-in').text('(¥0)');
    }
}
...
window.setUnitPriceHistory = function (target) {
    ...
    let row = $(target).closest('tr');
    let customerId = $('.input-customer-select option:selected').val();
    ...
    let url = $('.hidden-api-get-customer-price-url').val();
    axios.get(url, {
        params: {
...
window.store = function () {
    ...
    let sales_discount = $('.input-sales-discount').val().replace(/,/g, '');
    $('.input-sales-discount').val(sales_discount);
    ...
    $('#order_products_table tbody tr').each(function () {
        ...
        let purchaseUnitPrice = $(this).find('.input-unit-price-purchase').val().replace(/,/g, '');
        $(this).find('.input-unit-price-purchase').val(purchaseUnitPrice)
        let grossprofit = $(this).find('.input-product-sub-gross').val().replace(/,/g, '');
        $(this).find('.input-product-sub-gross').val(grossprofit);
    });
...
/**
 * 行の売上分類に応じて、数量・単価・金額の文字色を設定
 * 「返品」の場合は赤色、それ以外は黒色
 */
function setRowColorByClassification($row, classificationValue) {
    const isReturn = String(window.Laravel.enums.sales_classification.classification_return);
    let color;

    if (classificationValue === isReturn) {
        color = 'red';
    } else {
        color = 'black';
    }

    $row.find('.input-quantity').css('color', color);
    $row.find('.input-unit-price').css('color', color);
    $row.find('.input-product-sub-total').css('color', color);
}
