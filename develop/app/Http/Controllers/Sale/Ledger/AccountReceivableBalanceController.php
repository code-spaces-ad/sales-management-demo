...
use App\Http\Requests\Sale\Ledger\AccountReceivableBalanceSearchRequest;
use App\Models\Invoice\ChargeData;
use App\Models\Master\MasterCustomer;
use App\Models\Master\MasterHeadOfficeInfo;
use App\Models\Sale\Ledger\LedgerAccountReceivableBalance;
use Carbon\Carbon;
 ...
 * 売掛台帳画面用コントローラー
 */
class AccountReceivableBalanceController extends Controller
{
    /** 入力項目トレイト */
    use SessionConst;
    ...
    /**
     * AccountReceivableBalanceController constructor.
     */
    public function __construct()
    {
        ...
        // 売上計の行位置
        $this->sub_total_row = 54;
    }

    /**
     * Show the application dashboard.
     *
     * @param AccountReceivableBalanceSearchRequest $request
     * @return View
     */
    public function index(AccountReceivableBalanceSearchRequest $request): View
    {
        ...
        // 検索結果の取得
        $search_result = LedgerAccountReceivableBalance::getOrderPaginate($search_condition_input_data);
        ...
        return view('sale.ledger.accounts_receivable_balance.index', $data);
    }
     ...
     * @throws Exception
     */
    public function downloadExcel(AccountReceivableBalanceSearchRequest $request): StreamedResponse
    {
        $spreadsheet = $this->getSpreadSheet($request);

        // Excelファイル名
        $filename = Carbon::now()->format('YmdHis') . '_' .
            config('consts.excel.filename.ledger_accounts_receivable_balance');
     ...
     * @throws Exception
     */
    public function showPdf(AccountReceivableBalanceSearchRequest $request): RedirectResponse
    {
        $spreadsheet = $this->getSpreadSheet($request);
     ...
     * @throws Exception
     */
    private function getSpreadSheet(AccountReceivableBalanceSearchRequest $request): Spreadsheet
    {
        ...
        $path = storage_path(
            ...
            . config('consts.excel.template_file.ledger_accounts_receivable_balance')
        );
        ...
        foreach ($customers as $customer) {
            ...
            $search_result = LedgerAccountReceivableBalance::getOrder($search_condition_input_data);
            ...
            foreach ($search_result as $detail_data) {
                if ($detail_data->order_kind === OrderType::DEPOSIT) {
                    ...
                    $arrDeposit = LedgerAccountReceivableBalance::getDepositPaymentDetail1($detail_data);
    ...
    private function getActiveSheet(Spreadsheet $spreadsheet, Worksheet $sheet, $page_no, $customer): ?Worksheet
    {
        ...
        // シート名
        $sheet_name = '売掛台帳_' . $customer->code_zerofill . '_' . $page_no;
