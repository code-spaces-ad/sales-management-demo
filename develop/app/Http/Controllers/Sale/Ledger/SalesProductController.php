...
use App\Http\Requests\Sale\Ledger\SalesProductsSearchRequest;
use App\Models\Master\MasterHeadOfficeInfo;
use App\Models\Sale\Ledger\LedgerSalesProduct;
use Carbon\Carbon;
...
class SalesProductController extends Controller
{
     ...
     *
     * @param SalesProductsSearchRequest $request
     * @return View
     */
    public function index(SalesProductsSearchRequest $request): View
    {
        ...
        $data = [
            ...
            /** 検索結果 */
            'search_result' => [
                'order_details' => LedgerSalesProduct::getOrder($search_condition_input_data),
            ],
        ];

        return view('sale.ledger.sales_products.index', $data);
    }
     ...
     * @throws Exception
     */
    public function downloadExcel(SalesProductsSearchRequest $request): StreamedResponse
    {
        $spreadsheet = $this->getSpreadSheet($request);

        // Excelファイル名
        $filename = Carbon::now()->format('YmdHis') . '_' . config('consts.excel.filename.ledger_sales_product');
     ...
     * @throws Exception
     */
    public function showPdf(SalesProductsSearchRequest $request)
    {
        $spreadsheet = $this->getSpreadSheet($request);
        ...
        if ($ret !== 0) {
            ...
            return redirect(route('sale.ledger.sales_products'))
     ...
     * @throws Exception
     */
    private function getSpreadSheet(SalesProductsSearchRequest $request)
    {
        $search_condition_input_data = $request->validated();

        $path = storage_path(
            config('consts.excel.template_path')
            . config('consts.excel.template_file.ledger_sales_product')
        );
        ...
        // 表示データ取得
        $search_result = LedgerSalesProduct::getOrder($search_condition_input_data);
    ...
    private function setTitle($sheet)
    {
        ...
        $sheet->setCellValue($cell, '商品別売上表');
    ...
    private function setDetailData($sheet, $row_count, $order_detail)
    {
        ...
        $sheet->getStyle($cell)->getAlignment()->setHorizontal('left');
        $sheet->setCellValue("A{$row}", $order_detail->product_code);

        // 単位
        $cell = "C{$row}";
        $sheet->getStyle($cell)->getAlignment()->setHorizontal('right');
        $sheet->setCellValue("C{$row}", $order_detail->unit_name);
        ...
        $sheet->mergeCells($cell);
        $sheet->getStyle($cell)->getAlignment()->setHorizontal('left');
        $sheet->setCellValue("A{$row}", $order_detail->product_name);
     ...
     * @param $sub_total
     */
    private function setDetailTotalData($sheet, $sub_total)
    {
        // 税額
